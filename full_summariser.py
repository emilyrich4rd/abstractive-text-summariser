...
device = torch.device("cuda") # requires GPU to run
...
def initialise_model():
  if os.path.exists(model_path): # if the script has been run before, the model has been saved locally and can be loaded from local memory
    ...
    # imports Hugging Face's t5-base model, the lightweight version of Google's T5 model
    ...
    training_parameters = TrainingArguments(
      ...
      num_train_epochs = 3 # number of passes over the dataset during training
      per_device_train_batch_size = 8, #  training batch size per CPU/GPU
      per_device_eval_batch_size = 8, # evaluation batch size per CPU/GPU
